...
function mkAttrRegex(startDelim, endDelim) {
    ...
    return new RegExp(start + '\\s*(\'|"|&quot;|&#39;)(.*?)\\1\\s*\\|\\s*(?:tri\-translate|translate)\\s*(' + end + '|\\|)', 'g');
}
...
var Extractor = (function () {
    ...
    Extractor.prototype.extractJs = function (filename, src, lineNumber) {
        ...
        function isTriGetString(node) {
            return node !== null &&
                node.type === 'CallExpression' &&
                node.callee !== null &&
                node.callee.type === 'MemberExpression' &&
                node.callee.object !== null && (
                node.callee.object.name === 'triGettextCatalog' || (
                    // also allow gettextCatalog calls on objects like this.triGettextCatalog.getString()
                node.callee.object.property &&
                node.callee.object.property.name === 'triGettextCatalog')) &&
                node.callee.property !== null &&
                node.callee.property.name === 'getString' &&
                node.arguments !== null &&
                node.arguments.length;
        ...
        function isTranslateableValidationMessage(node, reference) {
            return node !== null &&
                node.type === 'ObjectExpression' &&
                node.properties[0] &&
                (node.properties[0].key.value === 'error' || node.properties[0].key.value === 'success');
        }
        ...
        walkJs(syntax, function (node, parentComment) {
            ...
            if (isGettext(node) || isGetString(node) || isTriGetString(node)) {
            ...
            } else if (isTranslateableValidationMessage(node, reference)) {
                str = node.properties[0].value.value;
            }
            if (str || singular) {
